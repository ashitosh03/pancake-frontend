...
import { useLac, useProfile } from 'hooks/useContract'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await lacContract.methods.allowance(account, profileContract.options.address).call()
      ...
      onApprove: () => {
        return lacContract.methods.approve(profileContract.options.address, allowance.toJSON()).send({ from: account })
      },
      onConfirm: () => {
